...
//    @IBOutlet weak private var videoFilterButton: UIButton!
    ...
    @IBOutlet weak private var videoBrighterButton: UIButton!
    
    private var videoBrighterOn: Bool = false
    ...
    private let filterRenderers: [FilterRenderer] = [RosyMetalRenderer(), RosyCIRenderer(), BrightenCIRenderer()]
    
    private let photoRenderers: [FilterRenderer] = [RosyMetalRenderer(), RosyCIRenderer(),BrightenCIRenderer()]
...
//        videoFilterButton.setImage(#imageLiteral(resourceName: "ColorFilterOff"), for: .normal)
        videoBrighterOn = false
        videoBrighterButton.setImage(#imageLiteral(resourceName: "ColorFilterOff"), for: .normal)
...
//        self.videoFilterButton.setImage(stateImage, for: .normal)
    ...
    @IBAction private func toggleBrightening() {
        
        videoBrighterOn = !videoBrighterOn
        let brightenEnabled = videoBrighterOn
        
        let stateImage = UIImage(named: brightenEnabled ? "ColorFilterOn" : "ColorFilterOff")
        self.videoBrighterButton.setImage(stateImage, for: .normal)
        
        let index = filterIndex+2
        ...
        // Enable/disable the video filter.
        // print("这里还是使用同步来关闭，异步的话，transformer模型关的太慢了!")
        if brightenEnabled {
            self.videoFilter = self.filterRenderers[index]
        } else {
            if let filter = self.videoFilter {
